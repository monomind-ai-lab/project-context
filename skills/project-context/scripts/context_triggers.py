...
def command_gate(hook: dict, target: Path, how: str) -> int:
    ...
    reason = "\n".join(
        [
            ...
            "result you have not generalised, an assumption you proceeded on —",
            "capture it instead of deciding now. It lands in inbox/ for",
            "promotion later, and costs one line:",
            "",
            "  python3 .agents/skills/project-context/scripts/context_capture.py \\",
            "      --kind decision|learning|question|assumption|constraint|proposal \\",
            "      --text \"<at most 200 words>\" --apply",
            "",
            "Where none fired, say so and record it instead of editing a file",
            "to silence this:",
            "",
